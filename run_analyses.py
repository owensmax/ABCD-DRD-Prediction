...
#################bivariate regressions for mri vars############

data_smri=load_data(get='smri',val_check=3)
data_dmri=load_data(get='dmri',val_check=3)
data_rs=load_data(get='rs',val_check=3)
data_sst=load_data(get='sst',val_check=3)
data_nb=load_data(get='nback',val_check=3)
data_mid=load_data(get='mid',val_check=3)
names = ["smri",'dmri','rsfmri','sst','nback','mid']
dtsets =[data_smri, data_dmri, data_rs, data_sst, data_nb, data_mid]
result_table_mri=pd.DataFrame(columns = ['B','SE','p','R2'])
for name, dtset in zip(names, dtsets):
    mri_vars = dtset[0].columns[dtset[0].columns.str.contains(name)]
    ivs = mri_vars
    i=0
    for iv in ivs:
        X = pd.to_numeric(data[iv])
        ...
        XY = XY.dropna()
        lm = scipy.stats.linregress(XY.iloc[:,0],XY.iloc[:,1])
        result_table_mri.loc[iv, 'B'] = lm[0]
        result_table_mri.loc[iv, 'SE'] = lm[4]
        result_table_mri.loc[iv, 'p'] = lm[3]
        result_table_mri.loc[iv, 'R2'] = lm[2]**2
        i += 1
result_table_mri = result_table_mri[~result_table_mri.index.str.contains("mean.motion")]
result_table_mri = result_table_mri[~result_table_mri.index.str.contains("_var_")]
result_table_mri = result_table_mri[~result_table_mri.index.str.contains("good_ser")]
result_table_mri = result_table_mri.sort_values('p', axis=0, ascending=True, inplace=False, 
                                                kind='quicksort', na_position='last', ignore_index=False, 
                                                key=None)
result_table_mri = result_table_mri.sort_values('p', axis=0, ascending=True, inplace=False, kind='quicksort', na_position='last', 
                      ignore_index=False, key=None)
fdr=sm.stats.multitest.fdrcorrection(result_table_mri['p'], alpha = 0.05, method='indep', is_sorted=False)
result_table_mri['fdr'] = fdr[0]
result_table_mri['fdr_p'] = fdr[1]
result_table_mri.to_csv('mri.csv')
